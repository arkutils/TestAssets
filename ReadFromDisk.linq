...
async Task Main()
{
    ...
    var allAssets = baseDirectory
        ...
        .Select(file =>
        {
            try
            {
                ...
                return new { Archive = reader.ReadArchive(), reader.RootElement };
            }
            catch (Exception ex)
            {
    ...
    DumpChildren(allAssets.First().RootElement);
}
...
public void DumpChildren(ArchiveElement root)
{
    var last = 0;
    ...
    {
		if(lastElement?.Parent != element.Parent)
		{
			if(lastElement == element.Parent)
			{
				indentLevel += 1;
			}
        ...
        var isLeaf = element.Children.Count == 0;

        if (element.StartPosition != last)
        {
			if(element.Parent?.Type != "Export")
			{
            	builder.Append("# ");				
			}
			
            builder.AppendLine($"UNKNOWN @ {last}");
            builder.AppendLine(BitConverter.ToString(root.Data[last..element.StartPosition]));
            last = element.StartPosition;
        }

		if (element.Parent?.Type == "Archive")
		{
			builder.AppendLine($"# {element.Name} @ {element.StartPosition}");
			indentLevel = 0;
		}
		else if (element.Parent?.Type == "Export" && element.Type == "Property[]")
		{
		...
		else if (isLeaf)
		{
            ...
            last = element.EndPosition;
		}
		else 
		{
            builder.AppendLine(indent + ReduceWhitespace($"{element.Type} {element.Name} {element.Value} @ {element.StartPosition}"));
        }
    ...
    if (last != root.Data.Length)
    {
        builder.AppendLine($"# UNKNOWN @ {last}");
        builder.AppendLine(BitConverter.ToString(root.Data[last..]));
    }
...
public class Archive
{
    ...
    public Archive Deserialize(ArchiveReader reader)
    {
        using (reader.StartElement("PackageFileSummary", "PackageFileSummary"))
        {
            ...
            Generations = reader.ReadList("Generations", GenerationCount, i => new GenerationInfo().Deserialize(reader, this, i));
            ...
            CompressedChunks = reader.ReadList("CompressedChunks", CompressedChunkCount, i => new CompressedChunk().Deserialize(reader, this, i));
...
public class GenerationInfo
{
    ...
    public GenerationInfo Deserialize(ArchiveReader reader, Archive archive, int arrayIndex)
    {
        NameCount = reader.ReadInt32("NameCount");
        ExportCount = reader.ReadInt32("ExportCount");
...
public class CompressedChunk
{
    ...
    public CompressedChunk Deserialize(ArchiveReader reader, Archive archive, int arrayIndex)
    {
...
public class ArchiveReader : IDisposable
{
    ...
    public List<T> ReadList<T>(string name, int count, Func<int, T> selector) => Read("List", name, () => Enumerable.Range(0, count).Select(selector).ToList());
