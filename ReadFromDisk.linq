...
async Task Main()
{
	...
	var loader = new ArchiveLoader(@"D:\repos\dotnetparser\TestAssets\Unreal\401");	

	var allAssets = await loader.Keys
		.Where(x => includes.Any(e => e.IsMatch(x) && !excludes.Any(e => e.IsMatch(x))))
		.MonitorProgressAsync(
			async file =>
			{
				...
				{
					await Task.Yield();
					var archive = loader[file];
					return archive;
				}
				catch (Exception ex)
				{
					ex.Message.Dump(file);
					return null;
				}
			},
			concurrencyLimit: 1)
...
public class Archive
{
	...
	public ArchiveElement ParseNotes { get; set; }
	...
	{
		using (reader.StartElement("PackageFileSummary"))
		{
			...
			Generations = reader.ReadList("Generations", GenerationCount, i => new GenerationInfo().Deserialize(reader, i));

			if (FileVersionUE >= 336)
			{
				SavedByEngineVersion = new EngineVersion().Deserialize(reader).ToString();
			}
			else
			{
			...
			{
				CompatibleWithEngineVersion = new EngineVersion().Deserialize(reader).ToString();
			}

			CompressionFlags = reader.ReadInt32("CompressionFlags");
			CompressedChunkCount = reader.ReadInt32("CompressedChunkCount");
			CompressedChunks = reader.ReadList("CompressedChunks", CompressedChunkCount, i => new CompressedChunk().Deserialize(reader, i));
		...
		if (FileVersionUE >= EngineVersions.VER_UE4_SERIALIZE_TEXT_INARCHIVES_459)
		{
			...
			GatherableTextData = reader.ReadList("GatherableTextData", GatherableTextDataCount, i => new GatherableTextData().Deserialize(reader));
		}
		...
		reader.ExecuteAtOffset(ImportOffset, () =>
		{
			...
			{
				reader.Read("Import", i.ToString(), () => Imports[i].Deserialize(reader));
			}
		});
		...
		reader.ExecuteAtOffset(ExportOffset, () =>
		{
			...
			{
				reader.Read("Export", i.ToString(), () => Exports[i].Deserialize(reader));
			}
		});
...
{
	private Lazy<Object> _lazyTarget;
	...
	//public string OuterName => Outer?.FullName;
	...
	public object Target => _lazyTarget.Value;

	public override string ToString() => FullName;

	public ObjectImport Deserialize(ArchiveReader reader)
	{
		var archive = reader.Archive;
		
		ClassPackage = reader.ReadName("ClassPackage");
		ClassName = reader.ReadName("ClassName");
		OuterIndex = reader.ReadInt32("OuterIndex");
		ObjectName = reader.ReadName("ObjectName");

		Outer = archive.GetImport(OuterIndex);

		_lazyTarget = new Lazy<object>(() =>
		{
			if (ClassPackage == "/Script/CoreUObject" && ClassName == "Package")
			{
				return reader.Loader[ObjectName];
			}

			if (Outer != null && Outer.ClassPackage == "/Script/CoreUObject" && Outer.ClassName == "Package")
			{
				var outerExports = (Outer.Target as Archive)?.Exports;
				var targetExport = outerExports?.FirstOrDefault(x => x.ObjectName == ObjectName);
...
public class ObjectExport : IObjectResource
{
	...
	public string DataHex => BitConverter.ToString(Data);

	IObjectResource IObjectResource.Outer => Outer;

	public override string ToString() => FullName;

	public ObjectExport Deserialize(ArchiveReader reader)
	{
		var archive = reader.Archive;
		
		ClassIndex = reader.ReadInt32("ClassIndex");
		SuperIndex = reader.ReadInt32("SuperIndex");

		if (archive.FileVersionUE >= 508)
		{
			TemplateIndex = reader.ReadInt32("TemplateIndex");
		}
		...
		Data = reader.ReadAtOffset("Byte[]", "Data", SerialOffset, () => reader.ReadBytes((int)SerialSize));

		Class = archive.GetImportOrExport(ClassIndex);
		Super = archive.GetImport(SuperIndex);
		Template = archive.GetImportOrExport(TemplateIndex);
		Outer = archive.GetExport(OuterIndex);

		Properties = reader.ReadAtOffset("Property[]", "Properties", SerialOffset, () => ReadProperties(reader, this));

		return this;
	}

	private List<Property> ReadProperties(ArchiveReader reader, ObjectExport export)
	{
		...
		while (reader.Position <= endOffset)
		{
			...
			{
				var property = new Property().Deserialize(reader, export);
...
public class GatherableTextData
{
	...
	public GatherableTextData Deserialize(ArchiveReader reader)
	{
...
public class GenerationInfo
{
	...
	public GenerationInfo Deserialize(ArchiveReader reader, int arrayIndex)
	{
		NameCount = reader.ReadInt32("NameCount");
		ExportCount = reader.ReadInt32("ExportCount");
...
public class CompressedChunk
{
	...
	public CompressedChunk Deserialize(ArchiveReader reader, int arrayIndex)
	{
...
public class NameIndex
{
	...
	public static implicit operator string(NameIndex obj) => obj.Value;

	public NameIndex Deserialize(ArchiveReader reader)
	{
		var archive = reader.Archive;
		
		Index = reader.ReadUInt32("Index");
		Instance = reader.ReadUInt32("Instance");
...
public class PackageIndex
{
	...
	public PackageIndex Deserialize(ArchiveReader reader)
	{
		...
		{
			var usedIndex = -Index - 1;
			if (usedIndex < reader.Archive.Imports.Count)
			{
				Referenced = reader.Archive.Imports[usedIndex];
			}
		}
		...
		{
			var usedIndex = Index - 1;
			if (usedIndex < reader.Archive.Exports.Count)
			{
				Referenced = reader.Archive.Exports[usedIndex];
			}
		}
...
public class PropertyTag
{
	...
	public PropertyTag Deserialize(ArchiveReader reader)
	{
		...
		var type = reader.ReadName("Type");

		Type = type;
		Size = reader.ReadInt32("Size");
		ArrayIndex = reader.ReadInt32("ArrayIndex");

		var engineVersion = reader.Archive.FileVersionUE;
		...
		if (type.Instance == 0)
		{
			...
			{
				StructName = reader.ReadName("StructName");
				if (engineVersion >= EngineVersions.VER_UE4_STRUCT_GUID_IN_PROPERTY_TAG_441)
				{
			...
			{
				EnumName = reader.ReadName("EnumName");
			}
			...
			{
				if (engineVersion >= EngineVersions.VAR_UE4_ARRAY_PROPERTY_INNER_TAGS_282)
				{
					InnerType = reader.ReadName("InnerType");
				}
			}
			else if (engineVersion >= EngineVersions.VER_UE4_PROPERTY_TAG_SET_MAP_SUPPORT_509)
			{
				if (Type == "SetProperty")
				{
					InnerType = reader.ReadName("InnerType");
				...
				{
					InnerType = reader.ReadName("InnerType");
					ValueType = reader.ReadName("ValueType");
				}
			}
		}

		// Property tags to handle renamed blueprint properties effectively.
		if (engineVersion >= EngineVersions.VER_UE4_PROPERTY_GUID_IN_PROPERTY_TAG_503)
		{
...
public class Property
{
	...
	public Property Deserialize(ArchiveReader reader, ObjectExport export)
	{
		...
		try
		{
			Tag = reader.Read("Tag", null, () => new PropertyTag().Deserialize(reader));
...
public class EngineVersion
{
	...
	public EngineVersion Deserialize(ArchiveReader reader)
	{
...
public class ArchiveLoader
{
	...
	private readonly ConcurrentDictionary<string, ArchiveReader> _readerCache;
	private readonly Lazy<Dictionary<string, FileInfo>> _assetFiles;
	
	public ArchiveLoader(string gamePath)
	{
		_gamePath = gamePath;
		_readerCache = new();
		_assetFiles = new(() => new DirectoryInfo(_gamePath)
			.EnumerateFiles("*.uasset", SearchOption.AllDirectories)
			.ToDictionary(x => FileSystemPathToAssetPath(x.FullName)));
	}

	public Dictionary<string, FileInfo> AssetFiles => _assetFiles.Value;
	
	public Archive this[string key] => TryGetValue(key, out var value) ? value : null;

	public IEnumerable<string> Keys => _assetFiles.Value.Keys;

	public IEnumerable<Archive> Values => throw new NotImplementedException();
	...
	public IEnumerator<KeyValuePair<string, Archive>> GetEnumerator()
	{
		throw new NotImplementedException();
	}

	public bool TryGetValue(string key, [MaybeNullWhen(false)] out Archive value)
	{
		var reader = _readerCache.GetOrAdd(key, path => _assetFiles.Value.TryGetValue(key, out var fileInfo) ? new ArchiveReader(key, fileInfo, this) : null);

		value = reader?.Archive;
		return value != null;
	}

	internal Archive LoadArchive(ObjectImport objectImport)
	{
		throw new NotImplementedException();
	}

	public string FileSystemPathToAssetPath(string path)
	{
		...
		var normalizedBasePath = Regex.Replace(_gamePath.Replace('\\', '/'), "/{2}", "/").TrimEnd('/');
	...
	public string AssetPathToFileSystemPath(string path)
	{
		var normalizedBasePath = Regex.Replace(_gamePath.Replace('\\', '/'), "/{2}", "/").TrimEnd('/');
...
public class ArchiveReader : IDisposable
{
	...
	public ArchiveReader(string name, FileInfo file, ArchiveLoader loader)
	{
		Loader = loader;
		_name = name;
	...
	public ArchiveLoader Loader { get; }
	...
	public Archive Archive
	{
		get
		{
			...
			using (var rootElement = StartElement(nameof(Archive), _name))
			{
				_archive = new Archive { Name = _name, ParseNotes = rootElement };
				rootElement.Value = _archive;
				return _archive.Deserialize(this);
			}
		}
	}
	...
	public T Read<T>(string type, string name, Func<T> reader)
	{
	...
	public NameIndex ReadName(string name) => Read("NameIndex", name, () => new NameIndex().Deserialize(this));

	public List<T> ReadList<T>(string name, int count, Func<int, T> selector) => Read("List", name, () => Enumerable.Range(0, count).Select(x => selector(x)).ToList());

	public string ReadString(string name)
	{
		return Read("String", name, () =>
		{
			var length = ReadInt32("Length");
	...
	{
		var bytes = ReadBytes(length * bytesPerChar, "Content");
...
public class ObjectResourceConverter : JsonConverter<IObjectResource>
{
	public override bool CanWrite => false;

	public override IObjectResource ReadJson(JsonReader reader, Type objectType, IObjectResource existingValue, bool hasExistingValue, JsonSerializer serializer)
	{
		...
		IObjectResource target = jObject.Property("SerialSize", StringComparison.OrdinalIgnoreCase) != null
			? new ObjectExport()
			: new ObjectImport();
	...
	public override void WriteJson(JsonWriter writer, IObjectResource value, JsonSerializer serializer)
	{
		throw new NotImplementedException();
	}
}
